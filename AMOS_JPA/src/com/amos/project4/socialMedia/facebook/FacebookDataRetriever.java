...
public class FacebookDataRetriever {
	...
	public static FacebookDataRetriever getInstance(){
		if(instance == null){
	...
	private void importFacebookData(User user,Client client, FacebookDataType type) {
			...
			if(user_ != null && user_.getId().length() > 0){
				...
				if(educations != null){
					for(com.restfb.types.User.Education edu : educations){
						saveFacebookData(client, edu.toString(), type);
					}
				}				
			}
			...
			if(user_ != null && user_.getId().length() > 0){
				...
				if(works != null){
					for(com.restfb.types.User.Work work : works){
						saveFacebookData(client, work.getPosition().getName() + "|" +work.getEmployer().getName(), type);
					}
				}				
			}
			return;
		case EVENTS:
			...
			if(events != null && events.size() > 0){
				for (FqlObject ev : events) {
					...
					System.out.println(tmp.getId()+"|"+tmp.getName()+"|"+tmp.getStartTime()+"|"+tmp.getEndTime()+"|"+tmp.getLocation()+"|"+tmp.getOwner()+"|"+tmp.getDescription());
				}
		...
		case LAST_POST:
			...
			if(rslts != null && rslts.size() > 0){
				for (FqlObject ev : rslts) {
					...
					//System.out.println(tmp.getId()+"|"+tmp.getUpdatedTime()+"|"+tmp.getMessage());
				}				
			}
			...
			if(rslts != null && rslts.size() > 0){
				...
				for (FqlObject ev : rslts) {
					...
					saveFacebookData(client, tmp.getId()+"|"+tmp.getCreatedTime()+"|"+tmp.getFrom().getId()+"|"+tmp.getTo().get(0)+"|"+tmp.getMessage(), type);
					System.out.println(tmp.getId()+"|"+tmp.getCreatedTime()+"|"+tmp.getMessage());
				}
