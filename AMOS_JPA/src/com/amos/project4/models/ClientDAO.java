...
public class ClientDAO {
	...
	public List<Client> getAllFilteredClients(SearchParameters search){
		if(search == null || search.getSearch_text() == "" || search.getSearch_text().equalsIgnoreCase("%")){
		...
		switch(search.getSearch_cat()){
			...
			case 3 :
				...
				break;
			case 4 :
				//q = em.createQuery("SELECT c FROM Client c WHERE c.birthdate = :param  ORDER BY c.ID");
...
//		 q.setParameter("id", client.getID());
		 Client _client = em.find(Client.class, client.getID()); //(Client) q.getSingleResult();
