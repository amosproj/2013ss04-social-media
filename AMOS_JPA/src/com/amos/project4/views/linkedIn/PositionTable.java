...
public class PositionTable extends JTable implements AbstractControlledView{
	...
	public PositionTable() {
		super();
		this.model = new PositionTableModel(new ArrayList<LinkedInData>());
		setModel(model);
	...
	public void modelPropertyChange(Observable o, Object arg) {
		if (arg != null && arg.getClass().equals(Client.class)) {
			...
			List<LinkedInData> tmp_name = c.getLinkedInDatasByType(LinkedInDataType.POSITIONS);			
			if(tmp_name != null && !tmp_name.isEmpty()){
				this.setModel((TableModel) new PositionTableModel(tmp_name));
			}else{
				this.setModel((TableModel) new PositionTableModel(new ArrayList<LinkedInData>()));
			}
			getTableHeader().getColumnModel().getColumn(0).setMaxWidth(300);
			getTableHeader().getColumnModel().getColumn(0).setMinWidth(200);
		}
